...
## Goal
建立一套可跨项目复用的 harness 基础协议，让 agent 在进入仓库后能通过 `AGENTS.md` 定位事实来源，并以 `docs/task-packages/<task>/` 作为唯一任务包完成设计、实现、验证与证据回写。
...
2. 任务的事实来源从 legacy task notes 切换为 `docs/task-packages/<task>/` 设计包。
