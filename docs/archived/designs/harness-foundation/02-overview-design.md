...
3. `.codex/skills/openharness/references/manifest.yaml` + `openharness` supporting files 作为执行入口
   - `.codex/skills/openharness/scripts/*` 是 harness 脚本来源。
...
- 未来迁移到别的项目时，只需要复制 `AGENTS.md` 结构约定、`.codex/skills/openharness/references/manifest.yaml` 和 skill，而不需要绑定到特定 legacy task notes 风格。
...
## Harness Commands
- `.codex/skills/openharness/scripts/openharness.py bootstrap`
  - 读取 manifest，列出 active design packages。
- `.codex/skills/openharness/scripts/openharness.py check-designs`
  - 校验 design package 必需文件和 `STATUS.yaml` 关键字段。
- `.codex/skills/openharness/scripts/openharness.py verify`
