# Evidence

## Files
- `docs/archived/task-packages/message-observability/README.md`
- `docs/archived/task-packages/message-observability/STATUS.yaml`
- `docs/archived/task-packages/message-observability/01-requirements.md`
- `docs/archived/task-packages/message-observability/02-overview-design.md`
- `docs/archived/task-packages/message-observability/03-detailed-design.md`
- `docs/archived/task-packages/message-observability/05-verification.md`
- `docs/archived/task-packages/message-observability/06-evidence.md`
- `docs/archived/legacy/or-task-007-message-observability-design.md`
- `docs/archived/legacy/or-task-007-message-observability-detailed-design.md`

## Commands
- `.agents/skills/openharness/using-openharness/scripts/openharness.py new-design message-observability OR-007 "Message Observability" --owner codex --summary "Formalize the landed message observability design as a harness package."`

## Follow-ups
- When legacy `docs/archived/legacy/` files are fully retired, replace them with archival pointers to this package.
