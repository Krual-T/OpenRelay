# Evidence

## Files
- `docs/designs/log-manager/README.md`
- `docs/designs/log-manager/STATUS.yaml`
- `docs/designs/log-manager/01-requirements.md`
- `docs/designs/log-manager/02-overview-design.md`
- `docs/designs/log-manager/03-detailed-design.md`
- `docs/designs/log-manager/05-verification.md`
- `docs/designs/log-manager/06-evidence.md`
- `docs/archived/legacy/or-task-014-log-manager-overall-design.md`

## Commands
- `.agents/skills/openharness/using-openharness/scripts/openharness.py new-design log-manager OR-014 "Log Manager" --owner codex --summary "Unify logger and observability into a structured system debug ledger entrypoint."`

## Follow-ups
- Write the real package-local detailed design before implementation begins.
- Keep this package, rather than any legacy note, as the canonical task fact source.
