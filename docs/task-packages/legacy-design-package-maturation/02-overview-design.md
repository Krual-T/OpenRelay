...
| --- | --- | --- | --- |
| `OR-013 Workspace Shortcuts And Directory Maintenance` | `STATUS.yaml` 已是 `in_progress`，已有代码和测试证据；`README.md` 仍写 `proposed` | 状态文档不一致，且卡片化维护路径是否已满足 done criteria 需要复核 | 先修状态一致性，再判断是否能进入 `verifying` |
...
降级方向：
- 如果一次性成熟化 4 个包成本过高，先只关闭 `OR-013` 的状态和验证债务，再依次推进 `OR-011`、`OR-010`、`OR-012`。
- 如果某个包在探索时发现范围过大，应在该包内拆新任务，不扩大 OR-017。

## Stage Gates
- 已列出 `OR-010` 到 `OR-013` 的成熟度、主要缺口和目标门槛。
- 已明确 `OR-014`、`OR-015`、`OR-018` 不属于本包范围。
- 已给出非编号排序的推荐顺序：`OR-013` 收口，`OR-011` 定状态模型，`OR-010` 收敛等待交互，`OR-012` 做异步回看。
- 已记录失败信号和降级方向。

## Trade-offs
备选方案一：按编号从 `OR-010` 顺序推进到 `OR-013`。

拒绝原因：编号顺序不能反映当前风险。`OR-013` 已经有实现和测试证据，先收口能减少一个活跃包；`OR-011` 的状态模型又会影响 `OR-010` 和 `OR-012`，应优先于它们成熟。
...
架构视角挑战：OR-013 已经是 `in_progress`，是否还应该算“历史设计成熟化”。处理结果：接受它仍在 OR-017 清单中，但只作为收口项，不再作为重设计项；它的下一步是修状态一致性并复核 done criteria。
